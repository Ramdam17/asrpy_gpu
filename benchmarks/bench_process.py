"""Benchmark ASR.transform / process across backends.

This is where the GPU is expected to win the most: ``process`` runs a
batched eigendecomposition over hundreds of windows, vs the per-window
Python loop in the numpy backend.
"""
...
logger = logging.getLogger("bench_process")
...
def bench_one(
    ...
    # Calibrate once with numpy (fast for small data) — measure transform only.
    M, T = bn.calibrate(X, sfreq=sfreq)

    if backend == "numpy":

        def fn():
            bn.process(X, sfreq=sfreq, M=M, T=T)
    else:
        from asrpy_gpu import _backend_torch as bt

        def fn():
            bt.process(X, sfreq=sfreq, M=M, T=T, device=device)
...
def main() -> None:
    ...
    for n_chan in cfg["channels"]:
        for dur in cfg["durations_s"]:
            for backend, device in targets:
                logger.info(
                    "benchmarking process: backend=%s device=%s n_chan=%d duration=%ds",
                    backend,
                    device,
                    n_chan,
                    dur,
    ...
    path = write_results_csv(rows, "bench_process.csv")
